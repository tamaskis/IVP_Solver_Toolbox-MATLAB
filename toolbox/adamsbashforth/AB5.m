...
%   [2] Montenbruck and Gill, "Satellite Orbits" (pp. 135-136)
%
%--------------------------------------------------------------------------
%
% ------
...
function [t,y] = AB5(f,I,y0,h,wb)
    ...
    if time_detection_implementation
        ...
        % propagating state vector using AB5
        for n = (m+1):N
            ...
            % state vector propagated to next sample time
            y(:,n+1) = y(:,n)+(h/720)*(1901*fm(:,m)-2774*fm(:,m-1)+2616*...
