%==========================================================================
%
% AB6  Propagates the state vector forward one time step using the
...
%
%==========================================================================
function F = AB6(f,t,F,h)
    
    % updates F matrix with new function evaluation
    F = [F(:,2:6),f(t,F(:,7)),F(:,7)];
    
    % state vector propagated to next sample time
    F(:,7) = F(:,7)+(h/1440)*(4277*F(:,6)-7923*F(:,5)+9982*F(:,4)-7298*...
        F(:,3)+2877*F(:,2)-475*F(:,1));
