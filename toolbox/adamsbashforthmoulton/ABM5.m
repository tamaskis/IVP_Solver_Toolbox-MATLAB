...
function [t,y] = ABM5(f,I,y0,h,wb)
    ...
    if time_detection_implementation
        ...
        % propagating state vector using ABM5
        for n = (m+1):N
            ...
            % predictor step
            yp = y(:,n)+(h/720)*(1901*fm(:,m)-2774*fm(:,m-1)+2616*...
                fm(:,m-2)-1274*fm(:,m-3)+251*fm(:,m-4));
            
            % corrector step (state vector propagated to next sample time)
            y(:,n+1) = y(:,n)+(h/720)*(251*f(t(n+1),yp)+646*fm(:,m)-...
                264*fm(:,m-1)+106*fm(:,m-2)-19*fm(:,m-3));
