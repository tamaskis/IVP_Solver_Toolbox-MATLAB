...
%
%==========================================================================
function [t,y] = RK4_38(f,I,y0,h,wb)
    ...
    if time_detection_implementation
        ...
        % propagating state vector using the 3/8-rule fourth-order method
        for n = 1:N
            ...
            k3 = f(tn+2*h/3,yn-h*k1/3+h*k2);
            k4 = f(tn+h,yn+h*k1-h*k2+h*k3);

            % state vector propagated to next sample time
            y(:,n+1) = yn+(h/8)*(k1+3*k2+3*k3+k4);
