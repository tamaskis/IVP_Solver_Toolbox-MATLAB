...
%   [t,y] = RK4(__,wb)
%% Description
% |[t,y] = RK4(f,[t0,tf],y0,h)| solves the ODE defined by |f(t,y)| from 
% |t0| until |tf| using the classic Runge-Kutta fourth-order method with an
% initial condition |y0| and step size |h|.
%%
% |[t,y] = RK4(f,{t0,C},y0,h)| does the same as the syntax above, but
% instead of terminating at a final time |tf|, the solver terminates once
% the condition function |C(t,y)| is no longer satisfied.
%%
% |[t,y] = RK4(...,wb)| can be used with either of the syntaxes above to
