%% |RK3_heun_step|
% Propagates the state vector forward one time step using Heun's 
% third-order method (Runge-Kutta third-order method).
% 
% <index.html Back to ODE Solver Toolbox Contents>.
%% Syntax
%   y_next = RK3_heun_step(f,t,y,h)
%% Description
% |y_next = RK3_heun_step(f,t,y,h)| returns the state vector at the next 
...
%
% _Find the solution $y(t)$ until $t=10$ using |RK3_heun_step|. Then, 
% compare your result to the solution found by |RK3_heun|._
%
...
t = (2:h:10)';
%%
% Solving for $y(t)$ using |RK3_heun_step| and comparing the result to the
% result obtained using |RK3_heun|,
...
% solving using "RK3_heun_step"
for i = 1:(length(t)-1)
    y(i+1) = RK3_heun_step(f,t(i),y(i),h);
end

% solving using "RK3_heun"
[t_RK3_heun,y_RK3_heun] = RK3_heun(f,[2,10],y2,h);

% maximum absolute error between the two results
max(abs(y_RK3_heun-y))
%%
