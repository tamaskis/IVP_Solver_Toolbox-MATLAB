...
%% Syntax
%   y_next = RK2_heun_step(f,t,y,h)
%% Description
% |y_next = RK2_heun_step(f,t,y,h)| returns the state vector at the next 
...
%
% _Find the solution $y(t)$ until $t=10$ using |RK2_heun_step|. Then, 
% compare your result to the solution found by |RK2_heun|._
%
...
t = (2:h:10)';
%%
% Solving for $y(t)$ using |RK2_heun_step| and comparing the result to the
% result obtained using |RK2_heun|,
...
% solving using "RK2_heun_step"
for i = 1:(length(t)-1)
    y(i+1) = RK2_heun_step(f,t(i),y(i),h);
end

% solving using "RK2_heun"
[t_RK2_heun,y_RK2_heun] = RK2_heun(f,[2,10],y2,h);

% maximum absolute error between the two results
max(abs(y_RK2_heun-y))
%%
