...
%% Description
% |y_next = RK3_ralston_step(f,t,y,h)| returns the state vector at the next 
...
%
% _Find the solution $y(t)$ until $t=10$ using |RK3_ralston_step|. Then, 
% compare your result to the solution found by |RK3_ralston|._
%
...
t = (2:h:10)';
%%
% Solving for $y(t)$ using |RK3_ralston_step| and comparing the result to
% the result obtained using |RK3_ralston|,
...
% solving using "RK3_ralston_step"
for i = 1:(length(t)-1)
    y(i+1) = RK3_ralston_step(f,t(i),y(i),h);
end

% solving using "RK3_ralston"
[t_RK3_ralston,y_RK3_ralston] = RK3_ralston(f,[2,10],y2,h);

% maximum absolute error between the two results
max(abs(y_RK3_ralston-y))
%%
