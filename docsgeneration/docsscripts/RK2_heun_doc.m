...
%   [t,y] = RK2_heun(__,wb)
%% Description
% |[t,y] = RK2_heun(f,[t0,tf],y0,h)| solves the ODE defined by |f(t,y)|
% from |t0| until |tf| using Heun's second-order method with an initial 
% condition |y0| and step size |h|.
%%
% |[t,y] = RK2_heun(f,{t0,C},y0,h)| does the same as the syntax above, but
% instead of terminating at a final time |tf|, the solver terminates once
% the condition function |C(t,y)| is no longer satisfied.
%%
% |[t,y] = RK2_heun(...,wb)| can be used with either of the syntaxes above
...
[t,y] = RK2_heun(f,[20,10],y20,0.001);
%%
...
[t,y] = RK2_heun(f,[10,20],y10,0.001);
y20 = y(end)
