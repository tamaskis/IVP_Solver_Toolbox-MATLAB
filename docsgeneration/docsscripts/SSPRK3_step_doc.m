%% |SSPRK3_step|
% Propagates the state vector forward one time step using the strong 
% stability preserving Runge-Kutta third-order method.
% 
% <index.html Back to ODE Solver Toolbox Contents>.
%% Syntax
%   y_next = SSPRK3_step(f,t,y,h)
%% Description
% |y_next = SSPRK3_step(f,t,y,h)| returns the state vector at the next 
...
%
% _Find the solution $y(t)$ until $t=10$ using |SSPRK3_step|. Then, compare
% your result to the solution found by |SSPRK3_step|._
%
...
t = (2:h:10)';
%%
% Solving for $y(t)$ using |SSPRK3_step| and comparing the result to the
% result obtained using |SSPRK3|,
...
% solving using "SSPRK3_step"
for i = 1:(length(t)-1)
    y(i+1) = SSPRK3_step(f,t(i),y(i),h);
end

% solving using "SSPRK3"
[t_SSPRK3,y_SSPRK3] = SSPRK3(f,[2,10],y2,h);
