%% |RK3_step|
% Propagates the state vector forward one time step using (Kutta's) 
...
%% Description
% |y_next = RK3_step(f,t,y,h)| returns the state vector at the next 
...
%
% _Find the solution $y(t)$ until $t=10$ using |RK3_step|. Then, compare
% your result to the solution found by |RK3_step|._
%
...
t = (2:h:10)';
%%
% Solving for $y(t)$ using |RK3_step| and comparing the result to the
% result obtained using |RK3|,
...
% solving using "RK3_step"
for i = 1:(length(t)-1)
    y(i+1) = RK3_step(f,t(i),y(i),h);
end

% solving using "RK3"
[t_RK3,y_RK3] = RK3(f,[2,10],y2,h);
