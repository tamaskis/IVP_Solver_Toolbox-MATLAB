...
% fourth-order method (Runge-Kutta fourth-order method).
% 
% <index.html Back to ODE Solver Toolbox Contents>.
%% Syntax
%   y_next = RK4_ralston_step(f,t,y,h)
%% Description
% |y_next = RK4_ralston_step(f,t,y,h)| returns the state vector at the next 
...
%
% _Find the solution $y(t)$ until $t=10$ using |RK4_ralston_step|. Then, 
% compare your result to the solution found by |RK4_ralston|._
%
...
t = (2:h:10)';
%%
% Solving for $y(t)$ using |RK4_ralston_step| and comparing the result to
% the result obtained using |RK4_ralston|,
...
% solving using "RK4_ralston_step"
for i = 1:(length(t)-1)
    y(i+1) = RK4_ralston_step(f,t(i),y(i),h);
end

% solving using "RK4_ralston"
[t_RK4_ralston,y_RK4_ralston] = RK4_ralston(f,[2,10],y2,h);

% maximum absolute error between the two results
max(abs(y_RK4_ralston-y))
%%
