%% |RK2_step|
% Propagates the state vector forward one time step using the midpoint
% method (Runge-Kutta second-order method).
% 
% <index.html Back to ODE Solver Toolbox Contents>.
%% Syntax
%   y_next = RK2_step(f,t,y,h)
%% Description
% |y_next = RK2_step(f,t,y,h)| returns the state vector at the next 
...
%
% _Find the solution $y(t)$ until $t=10$ using |RK2_step|. Then, compare
% your result to the solution found by |RK2_step|._
%
...
t = (2:h:10)';
%%
% Solving for $y(t)$ using |RK2_step| and comparing the result to the
% result obtained using |RK2|,
...
% solving using "RK2_step"
for i = 1:(length(t)-1)
    y(i+1) = RK2_step(f,t(i),y(i),h);
end

% solving using "RK2"
[t_RK2,y_RK2] = RK2(f,[2,10],y2,h);
